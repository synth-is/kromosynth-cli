...
export async function initializeKuzuDBWithFeatures(dbPath, detectedFeatureDimensions = new Set()) {
  ...
  const ENABLE_PROPERTY_INDEXES = process.env.KUZU_CREATE_INDEX === '1';
    ...
    // Optionally create indexes if explicitly enabled
...
// Extend schema for any new vector dimension not present at init time
async function extendSchemaForNewDimension(dbPath, dimension) {
  ...
  try {
    ...
    // Optionally create index for the new dimension if enabled
    const ENABLE_PROPERTY_INDEXES = process.env.KUZU_CREATE_INDEX === '1';
    if (ENABLE_PROPERTY_INDEXES) {
      try {
...
/**
 * Determine which vector property to use for a given dimension
 */
...
/**
 * Enhanced population function with feature vector support
 */
export async function populateKuzuDBWithLineageAndFeatures(evoRunConfig, evoRunId, lineageData, options = {}) {
  ...
  try {
  // First detect feature dimensions (initial guess)
  const detectedDimensionsInitial = await detectFeatureDimensions(evoRunConfig, evoRunId);
    
  // Initialize database with initially detected dimensions
  const initResult = await initializeKuzuDBWithFeatures(dbPath, detectedDimensionsInitial);
    ...
    // Initialize persistence provider for feature reading
    const persistenceProvider = new SQLitePersistenceProvider();
    
    // Get all genome IDs from lineage for feature lookup
    const genomeIds = lineageData.map(genome => genome.id).filter(id => id);
    console.log(`📊 Reading features for ${genomeIds.length} genomes...`);
    
    // Read features for all genomes at once (more efficient)
    ...
    // After loading all features, ensure schema has columns for every observed dimension
    const observedDims = new Set();
    for (const [gid, featsByType] of Object.entries(allFeatures)) {
      for (const [ftype, fdata] of Object.entries(featsByType)) {
    ...
    for (const genome of lineageData) {
      ...
      if (allFeatures[id]) {
        for (const [featureType, featureData] of Object.entries(allFeatures[id])) {
          if (featureData && featureData.features && Array.isArray(featureData.features)) {
            ...
            // Convert to KuzuDB vector format [val1, val2, ...]
            const vectorString = `[${featureData.features.join(', ')}]`;
            featureVectors[vectorProperty] = vectorString;
            
            // Use first feature type as primary, prioritize mfcc-sans0
      ...
      // Build dynamic INSERT query with available feature vectors
      // Determine a representative dimension (from primary feature type if available)
      ...
      if (allFeatures[id] && allFeatures[id][primaryFeatureType] && Array.isArray(allFeatures[id][primaryFeatureType].features)) {
        ...
        // fallback: pick any first dimension
...
export async function findSimilarSounds(dbPath, queryVector, featureType = 'mfcc-sans0', limit = 10, threshold = 0.8) {
  try {
    ...
    // Determine vector property based on query vector dimension
    const dimension = queryVector.length;
    const vectorProperty = getVectorPropertyName(dimension);
    
    // Convert query vector to KuzuDB format
...
/**
 * Get database stats including feature information
 */
  ...
  try {
    ...
    for (const dim of commonDimensions) {
      ...
      try {
        ...
        // Vector property doesn't exist
        vectorStats[`${dim}D`] = 0;
      }
    }
    
    // Generation and score stats
...
// ---------------------------------------------------------------------------
// Back-compat API: expose the legacy functions from this module as well, so
// callers can import a single integration regardless of feature usage.
// ---------------------------------------------------------------------------

export async function initializeKuzuDB(dbPath) {
  // Initialize with default known dimensions; behaves like legacy init, but with vector columns present.
...
export async function populateKuzuDBWithLineage(evoRunConfig, evoRunId, lineageData, options = {}) {
  // Route to the feature-aware population; if no features exist, it still inserts lineage-only nodes/edges.
