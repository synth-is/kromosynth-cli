...
export async function qdAnalysis_evoRunsFromDir(cli) {
  ...
  // Process each evolution run folder
  for (const evoRunFolder of evoRunFolders) {
    ...
    try {
      ...
      // Perform each analysis operation
      for (const oneAnalysisOperation of analysisOperationsList) {
        ...
        try {
          ...
          logError(`Error performing ${oneAnalysisOperation} analysis on ${evoRunFolder}: ${err.message}\n${err.stack}`);
        }
      }
